...
const CowCard: React.FC<{ 
  ...
  function getClassificationColor(classification: string | null) {
    switch (classification) {
      case 'Beef 1-3': return 'bg-[#dc2626]';
      case 'Beef 8-9': return 'bg-[#dc262680]';
      case 'Beef 4': return 'bg-[#ffce56]';
      case 'Beef 7': return 'bg-[#36a2eb]';
      case 'Beef 5': return 'bg-[#bbdd36]';
      case 'Beef 6': return 'bg-[#5a822b]';
      ...
      <div className="p-4">
        ...
        {cow.image_url && (
          <div className="w-full h-64 relative mt-2">
            <Image src={cow.image_url} alt={`Cow tag ${cow.breed}`} fill style={{ objectFit: 'contain' }} />
          </div>
        )}
        <div className='bg-brandLightGreen rounded-lg p-2 mt-2'>
          ...
          <div className="mt-4 flex justify-end space-x-2">
            <button onClick={() => onExpand(cow.id)} className="py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-brandGreen hover:bg-[#456422]">
              {isExpanded ? 'Hide Details' : 'Show Details'}
            </button>
            <button onClick={() => onDeleteCow(cow.id)} className="py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700">
              Remove from Pasture
            </button>
          </div>
        </div>
...
const PasturePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPastureData = async () => {
      try {
        ...
        setError('Error fetching pasture data');
        console.error(err);
      }
    };
  ...
  const filteredCows = pastureData ? (searchTerm ? pastureData.cows.filter(cow => cow.breed.toLowerCase().includes(searchTerm.toLowerCase())) : pastureData.cows) : [];
